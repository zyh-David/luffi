      ...
      <div class="main">
        <div class="course-info">
          ...
          <div class="wrap-right" v-if="course.discount_name">
            ...
            <div class="sale-time">
              ...
              <p class="expire">{{parseInt(course.activity_time/(3600*24)) | pad_0}}天 {{parseInt(course.activity_time/3600%24) | pad_0}}小时 {{parseInt(course.activity_time/60%60) | pad_0}}分  <span class="second">{{parseInt(course.activity_time%60) | pad_0}}</span> 秒</p>
            </div>
        ...
        <div class="course-content">
          <div class="course-tab-list">
            <div class="tab-item" v-if="tabIndex==1" v-html="course.brief">

            </div>
            <div class="tab-item" v-if="tabIndex==2">
              <div class="tab-item-title">
                <p class="chapter">课程章节</p>
                <p class="chapter-length">共{{course_chapters.length}}章 {{course.lessons}}个课时</p>
              </div>
              <div class="chapter-item" v-for="chapter in course_chapters">
                ...
                <ul class="lesson-list">
                  <li class="lesson-item" v-for="lesson in chapter.lesson_list">
                    <p class="name"><span class="index">{{chapter.chapter}}-{{lesson.id}}</span> {{lesson.name}} <span class="free" v-show="lesson.free_trail">免费</span></p>
...
export default {
    ...
    methods: {
      ...
      add_cart(){
        ...
        if(!token){
            //让客户确认是否登录
            this.$confirm('您尚未登录,请登录在进行操作！！','路飞学城',{
            ...
            this.$axios.post(`${this.$settings.Host}/cart/`,{
                ...
                console.log(response.data.cart_num)
                this.$message.success('添加商品到商品到购物车操作成功！');
                this.$store.commit(`update_num`, response.data.cart_num)
                sessionStorage.cart_num=response.data.cart_num
