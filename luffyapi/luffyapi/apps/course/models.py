...
class Course(BaseModel):
    ...
    course_img = models.ImageField(upload_to="course", max_length=255, verbose_name='课程封面', blank=True, null=True)
    ...
    period = models.SmallIntegerField(verbose_name='建议学习周期（day)', default=7)
    ...
    def real_price(self, expire_time):
        """根据有效期选项获取真实价格"""
        price = self.price
        # 如果购物车的有效期选项非0，则表示有其他有效期选项,并提取对应的价格
        if expire_time > 0:
            price = self.course_expire.get(expire_time=expire_time).price
        return price

    @property
    def discount_name(self):
        """当前课程参与的优惠类型"""
        # 判断当前课程是否参与了活动，如果有参与了活动，是否处于活动时间内
    ...
    def discount_price(self):
        ...
        sale = result.discount.sale     # 优惠公式
        if sale == "":
            """限时免费"""
            price = 0
...
class CourseExpire(BaseModel):
    ...
    @classmethod
    def get_expire_text(ctl, course_id, expire_time):
        """获取课程的有效时间的文本格式"""
        if expire_time>0:
...
class CourseDiscount(BaseModel):
    ...
    condition = models.IntegerField(blank=True, default=0, verbose_name='满足优惠的条件', help_text='设置参与优惠的价格门槛,表示商品必须在ｘｘ价格以上的价格以上的时候才参与优惠活动，<br>如果不填，则不设置门槛')
...
class Activity(BaseModel):
    ...
    remark = models.CharField(max_length=250, blank=True, null=True,verbose_name="优惠备注")
